 ...
 *
 * IMPORTANT: This is accessible ONLY via "test eod" command.
 * The production EOD flow (getEodRequestMessage in Templates.js) is NOT modified.
 * Form inputs use _extractFormInput() from ClickUpCards.js (robust for both Add-on and legacy formats).
 */
...
function _getEodState(email) {
  try {
    ...
    console.error('Error reading EOD state for ' + email + ':', e.message);
    return null;
  }
}
...
function _saveEodState(email, state) {
  try {
    ...
    if (json.length > 90000) {
      console.warn('EOD state too large for ' + email + ': ' + json.length + ' bytes, trimming tasks');
      // Trim clickUpTasks to fit within cache limit
      if (state.clickUpTasks && state.clickUpTasks.length > 5) {
        state.clickUpTasks = state.clickUpTasks.slice(0, 5);
        json = JSON.stringify(state);
      }
    }
    cache.put(key, json, EOD_CACHE_TTL);
  } catch (e) {
    console.error('Error saving EOD state for ' + email + ':', e.message);
  }
}
...
function _clearEodState(email) {
  try {
    ...
    console.error('Error clearing EOD state for ' + email + ':', e.message);
  }
}
...
function handleEodHeader(event) {
  ...
  // Use _extractFormInput (from ClickUpCards.js) for robust input extraction
  var totalHoursRaw = _extractFormInput(event, 'totalHours');
...
function handleEodTask(event) {
  ...
  // Extract form values using robust extraction
  var taskData = {
    taskName: _extractFormInput(event, 'taskName'),
    taskHours: _extractFormInput(event, 'taskHours'),
    carryOver: _extractFormInput(event, 'carryOver'),
    clickUpLink: _extractFormInput(event, 'clickUpLink'),
    noClickUpTask: _extractCheckboxInput(event, 'noClickUpTask'),
    taskStatus: _extractFormInput(event, 'taskStatus'),
    outcome: _extractFormInput(event, 'outcome'),
    progressPct: _extractFormInput(event, 'progressPct'),
    deliverableLink: _extractFormInput(event, 'deliverableLink'),
    blockerWhat: _extractFormInput(event, 'blockerWhat'),
    blockerOwner: _extractFormInput(event, 'blockerOwner'),
    blockerDeadline: _extractFormInput(event, 'blockerDeadline'),
    issueWhat: _extractFormInput(event, 'issueWhat'),
    issueAction: _extractFormInput(event, 'issueAction')
  };
  ...
  // Flag carry-over > 3 days for lead review
...
function handleEodMeetings(event) {
  ...
  var countRaw = _extractFormInput(event, 'meetingCount');
  var timeRaw = _extractFormInput(event, 'meetingTotalTime');
  var meetingCount = parseInt(countRaw) || 0;
  var meetingTotalTime = parseFloat(timeRaw) || 0;
  var meetingList = _extractFormInput(event, 'meetingList');
  var highMeetingNote = _extractFormInput(event, 'highMeetingNote');
...
function handleEodUnplanned(event) {
  ...
  var desc = _extractFormInput(event, 'unplannedDesc');
  var hoursRaw = _extractFormInput(event, 'unplannedHours');
  var pulledFrom = _extractFormInput(event, 'unplannedPulledFrom');
...
function handleEodTomorrow(event) {
  ...
  if (!confirmed) {
    var priorities = [];
    for (var i = 1; i <= 3; i++) {
      var name = _extractFormInput(event, 'priority' + i + 'Name');
      var link = _extractFormInput(event, 'priority' + i + 'Link');
      if (name) {
 ...
 * Supports both Workspace Add-on format and legacy Chat Bot format.
 */
function _extractCheckboxInput(event, fieldName) {
  try {
    var formInputs = (event.common && event.common.formInputs) ||
                     (event.commonEventObject && event.commonEventObject.formInputs);
    if (!formInputs || !formInputs[fieldName]) return false;

    var input = formInputs[fieldName];
    // Add-on format
    if (input.stringInputs && input.stringInputs.value) {
      return input.stringInputs.value[0] === 'true';
    }
    // Legacy format
    ...
    console.error('Error extracting checkbox "' + fieldName + '":', e.message);
  }
  return false;
}
...
function formatStructuredEodSummary(state) {
  ...
  (state.tasks || []).forEach(function(t) {
    ...
    if (t.carryOver && t.carryOver.indexOf('carry_') === 0) {
